...
//
// Samsung Galaxy Book3 Pro (Raptor Lake IPU6) variant of the mainline
// ov02c10 driver.  Three changes on top of upstream:
//
//  1) 26 MHz external clock support.  The firmware on these machines feeds
//     the sensor 26 MHz instead of the 19.2 MHz upstream assumes.  The PLL
//     register set is not re-tuned for it (the dividers have no exact
//     solution for 26 MHz), so *every* sensor clock comes out a factor
//     26/19.2 = 1.3542 too high:
//
//         MIPI link   400 MHz  ->  541.67 MHz  (1083 Mbps/lane)
//         pixel rate  160 MHz  ->  216.67 MHz  (2 lanes)
//         frame rate   30 fps  ->   40.8 fps
//
//     Upstream keeps advertising the nominal numbers, so the IPU6 CSI-2
//     receiver programs its D-PHY for 400 MHz while the sensor actually
//     transmits at 541.67 MHz.  That 35% mismatch is what produces the
//     "csi2-0 error: Frame sync error" messages and the random stalls.
//     We advertise the real frequencies instead, and stretch VTS so the
//     sensor really runs at 30 fps (which also gives back the full 33 ms
//     of integration time that the 40.8 fps overclock was stealing).
//
//  2) V4L2_CID_ANALOGUE_GAIN covers the sensor's digital gain as well
//     (1x .. 62x).  libcamera's software ISP only drives exposure and
//     analogue gain, and it truncates the raw data to 8 bits, so without
//     this the last stop of usable gain in low light is unreachable and
//     the picture posterises.  Values above 15.5x saturate the analogue
//     stage and put the remainder in the sensor's digital gain, which is
//     applied before the 10-bit output - i.e. before libcamera throws the
//     two low bits away.
//
//  3) Defaults tuned for libcamera's soft ISP AGC: analogue gain starts at
//     1x.  libcamera has no CameraSensorHelper for ov02c10, so it takes the
//     *default* value of V4L2_CID_ANALOGUE_GAIN as its "unity gain" point
//     and never goes below it.  A non-1x default therefore pins the AGC to
//     a permanent gain floor and forces it to shorten the exposure instead.
...
/* Frame rate the default VTS is computed for */
#define OV02C10_TARGET_FPS		30
...
/*
 * Analog gain controls from sensor.  Control units are 1/16 of a stop of
 * real gain, i.e. 0x10 == 1.0x, 0xf8 == 15.5x.
 */
...
/*
 * Digital gain from sensor, 0x400 == 1.0x.  There is no V4L2_CID_DIGITAL_GAIN
 * control any more: it is driven from V4L2_CID_ANALOGUE_GAIN so that a single
 * knob covers the whole gain range and nothing can leave a stale multiplier
 * behind.
 */
...
/*
 * V4L2_CID_ANALOGUE_GAIN is extended past the analogue stage: everything
 * above OV02C10_ANAL_GAIN_MAX is handed to the sensor's digital gain, up to
 * OV02C10_DGTL_GAIN_LIMIT.  15.5x analogue * 4x digital = 62x total.
 */
#define OV02C10_DGTL_GAIN_LIMIT		(4 * 1024)
#define OV02C10_TOTAL_GAIN_MAX \
	(OV02C10_ANAL_GAIN_MAX * (OV02C10_DGTL_GAIN_LIMIT / 1024))
...
/*
 * Camera modules that are physically mounted upside down.  The firmware on
 * these machines reports rotation 0 through the SSDB, so ipu-bridge hands us a
 * "rotation" property of 0 and nothing in the stack knows to flip the picture.
 *
 * The out-of-tree Intel driver worked around it by XOR-ing the mirror+flip bits
 * into 0x3820 at the end of start_streaming, behind the same module-name test.
 * That is the wrong layer for the mainline driver: libcamera writes
 * V4L2_CID_HFLIP/VFLIP itself, from the transform it derives from
 * V4L2_CID_CAMERA_SENSOR_ROTATION, so a rotation applied behind its back is
 * simply overwritten.  Report the rotation instead and let it do the flip.
 */
static const char * const ov02c10_inverted_modules[] = {
	"2BG203N3", "CJFME32", "KBFC645",
};

/* 822ace8f-2814-4174-a56b-5f029fe079ee: returns the camera module name */
static const guid_t ov02c10_module_guid =
...
MODULE_PARM_DESC(rotation,
		 "Sensor mounting rotation in degrees: -1 = autodetect (default), 0, or 180");
...
struct ov02c10 {
	...
	/* External clock rate, 19.2 MHz or 26 MHz */
	unsigned long mclk;
	/*
	 * Real link frequency, i.e. the nominal one scaled by mclk/19.2MHz.
	 * Backing store for the V4L2_CID_LINK_FREQ menu, so it has to live as
	 * long as the control handler does.
	 */
	s64 link_freq_real[ARRAY_SIZE(link_freq_menu_items)];
};
...
static int ov02c10_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_ANALOGUE_GAIN: {
		u32 again = min_t(u32, ctrl->val, OV02C10_ANAL_GAIN_MAX);
		u32 dgain;

		/*
		 * Saturate the analogue stage first, then spill the rest into
		 * the digital gain.  Digital gain is applied inside the sensor
		 * before the 10-bit output, so it still buys real tonal
		 * resolution for the 8-bit software ISP downstream.
		 */
		dgain = DIV_ROUND_CLOSEST(ctrl->val * 1024u, again);
		dgain = clamp_t(u32, dgain, OV02C10_DGTL_GAIN_MIN,
				OV02C10_DGTL_GAIN_LIMIT);
...
/*
 * Mounting rotation, in degrees. The "rotation" module parameter wins; failing
 * that, the ACPI module name identifies the known upside-down modules; failing
 * that, whatever the firmware said.
 */
static u32 ov02c10_rotation(struct ov02c10 *ov02c10, u32 fwnode_rotation)
{
	struct acpi_device *adev = ACPI_COMPANION(ov02c10->dev);
	union acpi_object *obj;
	unsigned int i;
	u32 ret = fwnode_rotation;

	if (rotation == 0 || rotation == 180) {
		dev_info(ov02c10->dev, "rotation forced to %d degrees\n", rotation);
		return rotation;
	}
	...
	obj = acpi_evaluate_dsm_typed(adev->handle, &ov02c10_module_guid,
				      0x00, 0x01, NULL, ACPI_TYPE_STRING);
	if (!obj)
		return ret;

	for (i = 0; i < ARRAY_SIZE(ov02c10_inverted_modules); i++) {
		if (!strcmp(ov02c10_inverted_modules[i], obj->string.pointer)) {
			dev_info(ov02c10->dev,
				 "camera module %s is mounted upside down, reporting 180 degrees\n",
				 obj->string.pointer);
			ret = 180;
			break;
		}
	...
	if (ret != 180)
		dev_info(ov02c10->dev, "camera module %s, rotation %u degrees\n",
			 obj->string.pointer, ret);
...
/*
 * Frame length that yields OV02C10_TARGET_FPS at the pixel rate this board's
 * external clock actually produces.
 *
 * This has to be used everywhere the blanking limits are (re)computed, not
 * just at control init: set_format() resets them too, and getting it wrong
 * there silently puts the sensor back to 40.8 fps for every application that
 * sets a format - which is all of them.
 */
			   ...
			   const struct ov02c10_mode *mode)
{
	...
	pixel_rate = div_u64(ov02c10->link_freq_real[ov02c10->link_freq_index] *
			     2 * ov02c10->mipi_lanes, OV02C10_RGB_DEPTH);
	vts = div_u64(pixel_rate, OV02C10_TARGET_FPS * mode->hts);

	return max_t(u32, vts, mode->vts_min * ov02c10->mipi_lanes);
}
...
static int ov02c10_init_controls(struct ov02c10 *ov02c10)
{
	...
	s64 exposure_max, exposure_def, h_blank, pixel_rate;
	unsigned int i;
	int ret;
	...
	/*
	 * The PLL register set is fixed, so all sensor clocks scale linearly
	 * with the external clock.  Advertise the frequencies the sensor
	 * really produces, otherwise the CSI-2 receiver programs its D-PHY
	 * for the wrong bitrate and the link becomes unreliable.
	 */
	for (i = 0; i < ARRAY_SIZE(link_freq_menu_items); i++)
		ov02c10->link_freq_real[i] =
			div_u64(link_freq_menu_items[i] * ov02c10->mclk,
				OV02C10_MCLK);
	...
	/*
	 * On a 26 MHz board this stretches VTS from 2328 to 3167 lines, taking
	 * the frame rate back down from 40.8 fps to 30 fps and handing the
	 * extra 8 ms to the exposure - free light, no extra gain, no noise.
	 */
	...
	/*
	 * Extended range: 1x .. 62x, analogue up to 15.5x and digital beyond
	 * that.  The default has to stay at 1x - libcamera's soft ISP treats
	 * the default as unity gain and will not go below it.
	 */
	v4l2_ctrl_new_std(ctrl_hdlr, &ov02c10_ctrl_ops, V4L2_CID_ANALOGUE_GAIN,
			  OV02C10_ANAL_GAIN_MIN, OV02C10_TOTAL_GAIN_MAX,
			  OV02C10_ANAL_GAIN_STEP, OV02C10_ANAL_GAIN_DEFAULT);

	exposure_max = vts_def - OV02C10_EXPOSURE_MAX_MARGIN;
	/*
	 * Start half way up the range instead of at the top.  The AGC walks
	 * the exposure in 10% steps, so an all-or-nothing starting point costs
	 * a couple of seconds of blown-out or black frames at every open.
	 */
	exposure_def = exposure_max / 2;
	...
	/*
	 * libcamera reads the *default* value of V4L2_CID_CAMERA_SENSOR_ROTATION
	 * to work out the mounting orientation, then sets HFLIP/VFLIP to correct
	 * it. So the rotation has to be fixed up here, before the control is
	 * created from props.
	 */
	props.rotation = ov02c10_rotation(ov02c10, props.rotation);
	...
	dev_info(ov02c10->dev,
		 "mclk %lu Hz, %u lanes, link-freq %lld Hz, pixel-rate %lld Hz, vts %u (%llu mfps), exposure max %lld lines (%llu us)\n",
		 ov02c10->mclk, ov02c10->mipi_lanes,
		 ov02c10->link_freq_real[ov02c10->link_freq_index],
		 pixel_rate, vts_def,
		 div_u64(pixel_rate * 1000, (u64)mode->hts * vts_def),
		 exposure_max,
		 div_u64((u64)exposure_max * mode->hts * 1000000, pixel_rate));
